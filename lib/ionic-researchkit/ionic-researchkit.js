        ...
        return {
            ...
            controller: ['$scope', '$rootScope', '$element', '$attrs', function($scope, $rootScope, $element, $attrs) {
                ...
                $scope.enableSlide = function (enable) {
                    $ionicSlideBoxDelegate.enableSlide(enable);
                };
        ...
        return {
            ...
            template: function (elem, attr) {
                return   	'<form name="form.'+attr.id+'"  novalidate>'+
                    ...
                    attr.min+
                    (attr.minText?'<br>'+attr.minText:'')+
                    '<input type="range" name="' + attr.id + '" min="' + attr.min + '" max="' + attr.max + '" step="' + attr.step + '" value="' + attr.value + '" on-touch="$parent.enableSlide(false)" on-release="$parent.enableSlide(true)" ng-model="$parent.formData.' + attr.id + '" ng-required="' + (attr.optional == 'false' ? 'true' : 'false') + '" ng-change="$parent.dirty()">' +
                    attr.max+
                    (attr.maxText?'<br>'+attr.maxText:'')+
                    '</div>'+
                    '</form>'
